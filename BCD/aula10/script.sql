...
CREATE FUNCTION formata_dinheiro(valor DECIMAL(10, 2))
RETURNS VARCHAR(15)
DETERMINISTIC
BEGIN
    RETURN CONCAT('R$ ', FORMAT(valor, 2, 'pt_BR'));
END //
...
SELECT 
    ...
    p.data,
    p.hora,
    formata_dinheiro(p.valor) AS valor_formatado
FROM 
    Pedidos p
JOIN 
...
BEGIN
    ...
    DECLARE cursor_pedidos CURSOR FOR SELECT id FROM Pedidos;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    
    OPEN cursor_pedidos;
    
    read_loop: LOOP
        FETCH cursor_pedidos INTO pedidoId;
        IF done THEN
            LEAVE read_loop;
        END IF;
        ...
        JOIN (
            SELECT 
                pedido_id,
                SUM(pizza.valor * ip.quantidade) AS total_valor
            FROM 
                Itens_Pedido ip
            JOIN 
                Pizzas pizza ON ip.pizza_id = pizza.id
            WHERE 
                ip.pedido_id = pedidoId
            GROUP BY 
                ip.pedido_id
        ) AS subquery ON p.id = subquery.pedido_id
        SET 
            p.valor = subquery.total_valor;
    END LOOP;
